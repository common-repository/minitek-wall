(function( $ ) {
	...
	$(function() {
        ...
        var navTabs = $( '#mwall-admin-metabox-wall-options-tabs').children( '.nav-tab-wrapper' ),
            tabIndex = null;
		...
		// Grab the wrapper for the Detail Box Tabs
        var dboxTabs = $( '#mwall-admin-metabox-detail-box-tabs').children( '.nav-tab-wrapper' ),
            dboxTabIndex = null;
 
        dboxTabs.children().each(function() {
            ...
            $( this ).on( 'click', function( evt ) {
                ...
                if ( ! $( this ).hasClass( 'nav-tab-active' ) ) {
                    ...
                    // Save the index of the tab that's just been marked as active.
                    dboxTabIndex = $( this ).index();
 
                    // Hide the old active content
                    $( '#mwall-admin-metabox-detail-box-tabs-content' )
                        .children( 'div:not( .hidden )' )
                        .addClass( 'hidden' );
  
                    // And display the new content
                    $( '#mwall-admin-metabox-detail-box-tabs-content' )
                        .children( 'div:nth-child( ' + ( dboxTabIndex + 1 ) + ')' )
                        .removeClass( 'hidden' );
